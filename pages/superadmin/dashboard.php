...
<?php
	...
	$jmlpeg	=mysql_query("SELECT * FROM tb_pegawai");
	$jpeg	=mysql_num_rows($jmlpeg);
			
	$jmlspd	=mysql_query("SELECT * FROM tb_spd");
	$jspd	=mysql_num_rows($jmlspd);
		
	$jmlkwi	=mysql_query("SELECT * FROM tb_kwitansi");
	...
	$total_dn	=mysql_query("SELECT SUM(jumlah) AS jml FROM tb_kwitansi WHERE ta IN (SELECT CAST(id_ta as CHAR) FROM tb_ta WHERE tahun='$now')  AND jns_tuj='Dalam Negeri'"); 
	$tot_dn		=mysql_fetch_assoc($total_dn);
	$jum_dn		=$tot_dn['jml'];
	$real_dn	=@($jum_dn/$tah['pagu_dn'])*100;
	$saldo_dn	=$tah['pagu_dn']-$jum_dn;
	
	$total_dk	=mysql_query("SELECT SUM(jumlah) AS jml FROM tb_kwitansi WHERE ta IN (SELECT CAST(id_ta as CHAR) FROM tb_ta WHERE tahun='$now') AND jns_tuj='Dalam Kota'"); 
		...
		<div class="row">
			<div class="col-md-4 col-sm-6">
				<div class="widget widget-stats bg-white text-inverse">
					<div class="stats-icon stats-icon-lg stats-icon-square bg-gradient-orange"><i class="ion-ios-personadd"></i></div>
			...
			<div class="col-md-4 col-sm-6">
				<div class="widget widget-stats bg-white text-inverse">
					<div class="stats-icon stats-icon-lg stats-icon-square bg-gradient-blue"><i class="fa fa-money"></i></div>
					<div class="stats-title">SPD RAMPUNG</div>
					<div class="stats-number"><?=$jspd?></div>
					<div class="stats-progress progress">
						<div class="progress-bar" style="width: 85%;"></div>
					</div>
					<div class="stats-desc">Total Data SPD Rampung</div>
				</div>
			</div>
		</div>
		<div class="row">
			...
			<div class="col-md-12">
				<div class="panel panel-inverse" data-sortable-id="index-1">
					<div class="panel-heading">
						...
						<h4 class="panel-title"><i class="ion-stats-bars fa-lg text-warning"></i> &nbsp;Statistik SPD <?php echo $now?></h4>
					</div>
	...
	var chart1; // globally available
		$(document).ready(function() {
			chart1 = new Highcharts.Chart({
				...
				title: {
					text: 'Statistik Per Bulan SPD <?php echo $now?>'
				},
				xAxis: {
					categories: ['Bulan']
				},
				...
				series:             
					[
					<?php 
					$sql   = "SELECT * FROM tb_spd WHERE ta='$tah[id_ta]' GROUP BY extract(month from tgl) ORDER BY tgl";
					$query = mysql_query( $sql )  or die(mysql_error());
						while( $ret = mysql_fetch_array( $query ) ){
								...
								$sql_jumlah   = "SELECT * FROM tb_spd WHERE tgl LIKE '%$new_tgl%'";        
