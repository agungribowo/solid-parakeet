<?php
	...
	$selTah	= $selTah	= mysql_query("SELECT SUM(pagu_ln) as `pagu_ln`, SUM(pagu_dn) as `pagu_dn`,SUM(pagu_dk) as `pagu_dk` FROM tb_ta WHERE tahun='$id_ta'");
	$tah	=mysql_fetch_array($selTah);
	
	$query	=mysql_query("SELECT * FROM tb_kwitansi WHERE ta IN (SELECT CAST(id_ta as CHAR) FROM tb_ta WHERE tahun='$id_ta') AND id_peg='$pegawai' ORDER BY id_kwitansi");
