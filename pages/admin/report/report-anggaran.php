<?php
	...
	$total_ln	=mysql_query("SELECT SUM(jumlah) AS jml FROM tb_kwitansi WHERE ta='$id_ta' AND jns_tuj='Luar Negeri'"); 
	$tot_ln		=mysql_fetch_assoc($total_ln);
	$jum_ln		=$tot_ln['jml'];
	$real_ln	=($jum_ln/$tah['pagu_ln'])*100;
	$saldo_ln	=$tah['pagu_ln']-$jum_ln;
	
	$total_dn	=mysql_query("SELECT SUM(jumlah) AS jml FROM tb_kwitansi WHERE ta='$id_ta' AND jns_tuj='Dalam Negeri'"); 
	$tot_dn		=mysql_fetch_assoc($total_dn);
	$jum_dn		=$tot_dn['jml'];
	$real_dn	=($jum_dn/$tah['pagu_dn'])*100;
	$saldo_dn	=$tah['pagu_dn']-$jum_dn;
	
	$total_dk	=mysql_query("SELECT SUM(jumlah) AS jml FROM tb_kwitansi WHERE ta='$id_ta' AND jns_tuj='Dalam Kota'"); 
		...
		<div class="profile-container">
							...
							<div class="col-md-5">
								<?php
									...
									while ($rownom = mysql_fetch_array($nompeg)) { 
										...
										echo '<option value="'.$rowpeg['id_peg'].'">'.$rowpeg['nama'].' _ '.$rowpeg['nip'].'</option>';    
										}    
									}    
									echo '</select>';
								?>
			...
			<div class="profile-section">
				<table id="data-table" class="table table-striped table-bordered nowrap" width="100%">
					...
					<tbody>
						<?php
							$query	=mysql_query("SELECT * FROM tb_kwitansi WHERE ta='$id_ta' ORDER BY id_peg");
							$no=0;
