...
?>
 ...
 <div class="col-md-3">
  ...
  <input type="text" name="tgl_berangkat" class="form-control" value="<?=$data['nomor_jaldis']?>" readonly="readonly">      
  </div>
</div>
...
<script>
 ...
 "<div class='form-group col-md-3'>" +
  "<label>Nomor Jaldis</label>" +
   "<input type='text' value='<?php echo ' '.$data['nomor_jaldis'].' '; ?>' name='nomor_berangkat' class='form-control' readonly='readonly'>      "+
 "  </div>"+
  ...
  "<label>Pegawai</label>" +
   ...
   " <?php 
     $q = mysqli_query($koneksi, 'SELECT * FROM pegawai_bnpt');
    while($data = mysqli_fetch_array($q)){
     echo '<option value='.$data['nama_pegawai'].'>'.$data['nama_pegawai'].'('.$data['nip'].' '.$data['golongan'].' ('.$data['jabatan'].'))</option>';
